...
import static com.dlqudtjs.codingbattle.common.constant.Destination.*;
...
public class JudgeController {
	...
	public ResponseEntity<ResponseDto> judge(
		@Valid @RequestBody JudgeProblemRequestDto judgeProblemRequestDto,
		@RequestHeader("Authorization") String token) {
		if (!jwtTokenProvider.getUserName(token)
			.equals(judgeProblemRequestDto.getUserId())) {
			throw new Custom4XXException(INVALID_INPUT_VALUE.getMessage(),
				INVALID_INPUT_VALUE.getStatus());
		}

		ResponseDto responseDto = judgeService.judgeProblem(
			judgeProblemRequestDto);
		return ResponseEntity.status(responseDto.getStatus()).body(responseDto);
	}

	@PostMapping("/v1/judges/results")
	public ResponseEntity<ResponseDto> judgeResults(
		@RequestBody JudgeResultRequestDto JudgeResultRequestDto) {
		// secret key 검증
		...
		if (isFinished) {
			submitService.saveSubmitResult(
				JudgeResultRequestDto.toParsedJudgeResultResponseDto());
		}
	...
	private Boolean isFinished(JudgeResultRequestDto result) {
		return !result.getSubmitResult()
			.getName()
			.equals(SubmitResultManager.PASS.getName()) ||
